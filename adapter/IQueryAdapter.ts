...
export interface IQueryAdapter extends IAdapter {
     ...
     * @returns {Promise<Record<string, unknown>[] | { items: Record<string, unknown>[]; total: number } | number>} - Array of query results, a { items, total } object for paged queries, or an HTTP status number if an error
     */
    runQuery: (query: string, variables: Record<string, unknown>, take?: number, skip?: number) => Promise<Record<string, unknown>[] | { items: Record<string, unknown>[]; total: number } | number>;
    /**
